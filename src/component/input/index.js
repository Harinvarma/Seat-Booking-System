...
const InputSeat = (props) => {
  ...
  const onChanegHandle = (e) => {
    if (e.target.value === "") {
      ...
      if (temp === 0) {
        ...
        setError( `Maximum ${seatSpecification.maxSeatAllowed} can be selected at once` );
        setRequiredSeats(temp);
      } else if (temp < 0){
        setRequiredSeats(" ")
      }
  ...
  return (
    <div>
      <div className="input">
        <input type="number" onChange={onChanegHandle} placeholder="Enter the number of seats you would like to book " value={requiredSeats}></input>
        <span className={!error && requiredSeats ? "active" : "de-active"}onClick={onClickHandle}>
          BookSeat
        </span>
      </div>
